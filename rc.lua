...
mymainmenu = awful.menu({ items = { { "awesome", myawesomemenu, beautiful.awesome_icon },
                                    ...
                                    { "file manager", function () awful.util.spawn("pcmanfm") end },
...
-- Define the shell command to get the volume
local cmd = [[bash -c "getvolume"]]

-- Use gears.timer to refresh the volume widget every 2 seconds
local gears = require("gears")
local timer = gears.timer {
    timeout = 3,  -- Set the interval in seconds
    call_now = true,
    autostart = true,
    callback = function()
        awful.spawn.easy_async_with_shell(cmd, function(out)
            volumewidget:set_markup(string.format("Vol:%s%% ", out))
        end)
    end
}
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey },            "b",     function () awful.util.spawn("brave") end,
              {description = "launch Brave Browser", group = "application"}),

    -- File manager
    awful.key({ modkey, "Shift"  },  "f",     function () awful.util.spawn("pcmanfm") end,
              {description = "show file manager", group = "application"}),
...
awful.spawn.with_shell("xrandr --output HDMI1 --auto --left-of LVDS1")
